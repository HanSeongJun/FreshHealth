...
for _, row in df.iterrows():
    # 엑셀 데이터로 템플릿 문자열 채우기
    marker_code = marker_template.replace('#이름', row['이름'])
    marker_code = marker_code.replace('#식당이름_변수명', row['식당이름_변수명'])
    marker_code = marker_code.replace('#위도', str(row['위도']))
    marker_code = marker_code.replace('#경도', str(row['경도']))
    marker_code = marker_code.replace('#번호', str(row['번호']))
    marker_code = marker_code.replace('#도로명주소', row['도로명주소'])
    marker_code = marker_code.replace('#번지주소', row['번지주소'])
    marker_code = marker_code.replace('#식당종류', row['식당종류'])
    marker_code = marker_code.replace('#네이버지도링크', row['네이버지도링크'])

    result.append(marker_code)
